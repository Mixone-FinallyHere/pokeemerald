...
static void GetTMNumberAndMoveString(u8 * dest, u16 itemId)
{
    ...
    if (itemId >= ITEM_HM01 && itemId <= (ITEM_HM01 + NUM_HIDDEN_MACHINES - 1))
    {
    ...
    else
    {
        ...
        {
            index = itemId - ITEM_TM01;
        }
        else if (itemId >= ITEM_TM51 && itemId <= ITEM_TM60)
        {
            index = NUM_ORIGINAL_TMS + (itemId - ITEM_TM51);
        }
        StringAppend(gStringVar4, gText_NumberClear01);
        ConvertIntToDecimalStringN(gStringVar1, index+1, STR_CONV_MODE_LEADING_ZEROS, 2);
        StringAppend(gStringVar4, gStringVar1);
    }
